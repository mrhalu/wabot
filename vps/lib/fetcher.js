...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./database/media/logo.jpg')
...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./database/dbmedia/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './database/data.exif', media, '-o', out])
        .on('exit', () => {
